...
class WeiboPage():
    ...
    def get_page(self, p1, p2):
        ...
        try:
            ...
            if len(one.items()) != 0:
                ...
                all_topic = self.get_topic(one.keys(), one.values())  # 列表元组

                topic_keywords = []
                topic_list = []
                topic_id_list = []

                for t in range(len(all_topic)):  # 话题
                    one_topic_key = Keyword().combine_keywords(all_topic[t][0])  # id

                    topic_list.append(all_topic[t][0])
                    topic_keywords.append(one_topic_key)
                    topic_id_list.append(all_topic[t][1])

                tp_and_tpw = zip(topic_list, topic_keywords, topic_id_list)  # 话题,话题关键字,话题id

                for one_tuple in tp_and_tpw:
    ...
    def get_topic(self, key, value):
        ...
        topic_bid = zip(weibo_topic, blog_id_list)

        print 'weibo长度', len(weibo_topic)
        print 'id长度', len(blog_id_list)
